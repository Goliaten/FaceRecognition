...
Call this file in the following way:
    ...
    python train.py name_of_experiment --images="\path\to\lfwcrop_grey\faces"

or more complex:

    python train.py name_of_experiment
           --images="\path\to\lfwcrop_grey\faces"
...
#from keras.src.layers.merging.base_merge import _Merge as merge
...
def create_model():
    ...
    def conv(x, n_filters, kH, kW, sH, sW, border_same=True, drop=0.0,
             sigma=0.0):
        border_mode = "same" if not border_same else "valid"
        x = Convolution2D(n_filters, (kH, kW), strides=(sH, sW),
                          padding=border_mode, kernel_initializer=init_conv, data_format='channels_last')(x)
        x = LeakyReLU(0.33)(x)
        if sigma > 0:
    ...
    #                output_shape=(512,))
    merged = Subtract()([face_left, face_right])
    merged = Lambda(lambda x: tf_abs(x))(merged)
     
    merged = GaussianNoise(0.5)(merged)
    merged = Dropout(0.2)(merged)

    merged = Dense(256, kernel_initializer=init_dense)(merged)
    ...
    classification_model = Model([face_left_input, face_right_input],
                                 merged)
    ...
    for epoch in range(epoch_start, EPOCHS):
        ...
        if swae and swae > 0 and (epoch+1) % swae == 0:
            ...
            if epoch % 10 == 0 and epoch > 1:
                weights_fname = f"{identifier}.{epoch}.last.weights"
                weights_fp = os.path.join(SAVE_WEIGHTS_DIR, weights_fname + str(epoch))
                model.save_weights(weights_fp, overwrite=True)
...
def ask_continue(message):
    ...
    while choice not in ["y", "n"]:
        choice = input("Enter 'y' (yes) or 'n' (no) to continue.")
    return choice == "y"
