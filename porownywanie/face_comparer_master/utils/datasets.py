...
class ImageFile(object):
    ...
    def get_content(self):
        ...
        if IMAGE_CHANNELS == 3:
            img = imageio.imread(self.filepath, mode="RGB")
        else:
...
class ImagePair(object):
    ...
    def get_contents(self, height, width):
        ...
        if img1.shape[0] != height or img1.shape[1] != width:
            ...
            if IMAGE_CHANNELS == 1:
                img1 = np.resize(np.squeeze(img1), (height, width))
                img1 = img1[:, :, np.newaxis]
            else:
                img1 = np.resize(img1, (height, width))
        if img2.shape[0] != height or img2.shape[1] != width:
            if IMAGE_CHANNELS == 1:
                img2 = np.resize(np.squeeze(img2), (height, width))
